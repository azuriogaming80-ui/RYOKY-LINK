...
const playButtonLabel = computed(() => {
  ...
  if (itemStore.isSeries) {
    const resumeEp = itemStore.episodes.find(
      ep => ep.userData?.playbackPositionTicks && ep.userData.playbackPositionTicks > 0
    );
    const nextEp = itemStore.episodes.find(
      ep => !ep.userData?.played && ep.userData?.playbackPositionTicks === 0
    );
  ...
  if (itemStore.item.userData?.playbackPositionTicks && itemStore.item.userData.playbackPositionTicks > 0) {
    return 'Reprendre';
  }
...
function handlePlay() {
  ...
  if (itemStore.isMovie || itemStore.isEpisode) {
    router.push({
      name: 'player',
      query: { itemId: itemStore.item.id },
    });
    return;
  }
